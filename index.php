...
$sql = "select * from `admins` where phone = '{$_GET['phone']}' and password='{$_GET['password']}'";
$res = mysqli_query($con, $sql);

for ($data = []; $v = $res->fetch_assoc(); $data[] = $v);

$sql5 = "select * from `teachers` where phone = '{$_GET['phone']}' and password='{$_GET['password']}'";
$res5 = mysqli_query($con, $sql5);

for ($data5= []; $v = $res5->fetch_assoc(); $data5[] = $v);
if (isset($_GET["s1"])) {
    session_start();
    if (!empty($data)) {
        $_SESSION["login"] = true;
        ...
        if (mysqli_num_rows($res2) == 0) {
            ...
            $sql3 = "insert into `imgs`(user_id,img) values('$se','$img')";
            mysqli_query($con, $sql3);

        } else {

            $sql4 = "select * from `imgs` where user_id = '$se'";
